...
def delete(key):
  ...
  # 1. 子を持たない場合
  if tree[key].left == None and tree[key].right == None:
    if parent < key:
  ...
  # 2. 子を1つ持つ場合
    ...
    if parent < key:
      tree[parent].right = tree[key].left
    else:
      tree[parent].left = tree[key].left
    tree[tree[key].left].parent = parent
    del tree[key]
  elif tree[key].left == None:
    if parent < key:
      tree[parent].right = tree[key].right
    else:
      tree[parent].left = tree[key].right
    tree[tree[key].right].parent = parent
    del tree[key]
  # 3. 子を2つ持つ場合
  else:
    left = tree[key].left
